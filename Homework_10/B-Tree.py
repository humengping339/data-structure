...
class BTree:
    ...
    # 新增：带斜线可视化打印
    def print_graphic_tree(self):
        root = self.root
        # 打印根节点行
        root_key_str = f"[{', '.join(map(str, root.keys))}]"
        root_text = f"{root_key_str} 关键字数={len(root.keys)}，子节点数={len(root.children)}"
        print(" " * 16 + root_text)
        # 打印分隔斜线 /  |  \
        print(" " * 13 + "/     |     \\")
        # 打印一层叶子节点
        ...
        for nd in child_nodes:
            key_count = len(nd.keys)
            child_count = len(nd.children)
            key_str = f"[{', '.join(map(str, nd.keys))}]"
            tag = f"(键{key_count},子{child_count})"
            output_parts.append(f"{key_str} {tag}")
